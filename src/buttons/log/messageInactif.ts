...
export const data = {
  name : "embeds.logs.message.inactif"
}
...
export const main = async (interaction: ButtonInteraction) => {
  ...
  const logChannel = guild.channels.cache.find((ch) => {
    ...
    return (ch as TextChannel).topic?.includes(IC_ZeroWidthSpace);
  ...
  const messageLogChannel = channels.find(ch => ("topic" in ch) && ch.topic?.includes(IC_ZeroWidthSpace))?.delete("");
  if(!messageLogChannel) return;

  const hasMessageLog = false;
