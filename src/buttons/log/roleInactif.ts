...
export const data = {
  name : "embeds.logs.roles.inactif"
}
...
export const main = async (interaction: ButtonInteraction) => {
  ...
  const messageLogChannel = channels.find(ch => ("topic" in ch) && ch.topic?.includes(IC_ZeroWidthNonJoiner))?.delete("");
  if(!messageLogChannel) return;

  const hasMessageLog = channels.some(ch => ("topic" in ch) && ch.topic?.includes(IC_ZeroWidthSpace));
  const hasRoleLog = false;
