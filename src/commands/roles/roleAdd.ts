...
export const data = new SlashCommandBuilder()
  .setName("role-add")
  .setNameLocalizations({ fr: "role-ajouter" })
  .setDescription("Add a role to all server members")
  .setDescriptionLocalizations({ fr: "Ajouter un rôle à tous les membres du serveur" })
  .addRoleOption((option) =>
    option
      .setName("role")
      .setDescription("The role to add to everyone")
      .setDescriptionLocalizations({ fr: "Le rôle à ajouter à tout le monde" })
      .setRequired(true),
  )
...
export const main = async (interaction: ChatInputCommandInteraction) => {
  ...
  const startedEmbed = infoEmbed({
    description: t(lang, "roles.add_started", { role: role.toString() }),
    lang,
  });
  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    const results = await Promise.allSettled(batch.map((m) => m.roles.add(role.id)));
    ...
    if (batchCount % PROGRESS_INTERVAL === 0 && processed < total) {
      const progressEmbed = infoEmbed({
        description: t(lang, "roles.add_progress", {
  ...
  const doneEmbed = successEmbed({
    description: t(lang, "roles.add_complete", {
      role: role.toString(),
